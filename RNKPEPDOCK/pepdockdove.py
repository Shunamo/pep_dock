...
# pepflow 실행 함수. num_models개의 conformation 생성
# out_dir: ./output/models/{sequence}_{model_num}.pdb
...
# pdb 파일에서 MODEL ... ENDMDL 블록을 잘라서 models/{sequence}_{model_num}.pdb로 저장
...
# megadock 실행 함수. num_decoys개의 decoy 생성 한 모델당 하나의 도킹 실행
# input: models/{sequence}_{idx}.pdb (peptide 모델)
# output: decoys/{sequence}_{idx}/{sequence}_{idx}.decoy_{i}.pdb (decoy)
...
# megadock multiprocessing wrapper
...
# decoys dir을 gnn_dove에 전달해서 reranking
...
# CLI 인자로 sequence, receptor 경로, 모델 수, decoy 수, output 경로 받아서 실행
def main():
